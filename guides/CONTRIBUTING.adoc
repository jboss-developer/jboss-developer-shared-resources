...
. To verify if your code followed the general guidelines you can run http://jboss-developer.github.io/maven-qstools-plugin/[QS Tools] on your project.
+
...
----
+
This generates a report in `__QUICKSTART_HOME__/target/site/qschecker.html`. Review the report to determine if your quickstart project violates any item in the xref:general_guidelines[General Guidelines].
...
* You should be able to import the sample project into Red Hat JBoss Developer Studio/JBoss Tools and deploy it from there.
